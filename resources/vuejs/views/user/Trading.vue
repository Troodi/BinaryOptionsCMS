<template>
  <div class="content-fluld" :style="{ 'margin-top': '4.6rem' }">
    <!-- Modal discount -->
    <div class="modal fade" id="greatWorkModal" tabindex="-1" role="dialog" aria-labelledby="discountModalLabel"
         aria-hidden="true">
      <div class="modal-dialog gradient-border" role="document" style="top:25vh">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" @click="moveToReal" class="btn btn-warning w-100">{{
                $i18n.t('trade_go_real')
              }}
            </button>
          </div>
        </div>
            ...
            <div class="card-content">
              <div class="card-body">
                <div class="card-text">
                  ...
                  <div v-show="number_percent !== null && number_percent > 0 && isButtonDisabled"
                       class="alert bg-rgba-danger alert-dismissible mb-2" role="alert">
                    <div class="d-flex align-items-center">
                      {{ $i18n.t('trade_expiration_min', {min_expiration: formatSeconds()}) }}
                    </div>
                  </div>

                  <small class="text-muted"><i>{{ $i18n.t('trade_expiration') }} <span v-show="number_percent !== null">({{
                      $i18n.t('trade_expiration_min', {min_expiration: formatSeconds()})
                    }})</span></i></small>
                  <fieldset class="form-group position-relative" :style="{ 'margin-bottom': '0.3rem !important'}">
                    <input id="time" @click="timeClick" readonly="readonly" type="text" style="opacity:1;"
                           class="form-control form-control-lg" v-model="expiration">
                  ...
                  <div class="row pb-1" v-show="clicked">
                    <div class="col-4 pr-0">
                      ...
                      <div class="d-inline-block w-100">
                        <div class="input-group bootstrap-touchspin bootstrap-touchspin-injected">
                          <input @focus="$event.target.select()" @click="$event.target.select()" type="number"
                                 class="touchspin-vertical form-control" v-model="hours">
                          <span class="input-group-btn-vertical">
                                                <button @click="hourAdd" style="width: 20px;"
                                                        class="btn btn-primary bootstrap-touchspin-up"
                                                        type="button">+</button>
                                                <button @click="hourSub" style="width: 20px;"
                    ...
                    <div class="col-4 pr-0" style="padding-left:7.5px;padding-right:7.5px !important;">
                      ...
                      <div class="d-inline-block w-100">
                        <div class="input-group bootstrap-touchspin bootstrap-touchspin-injected">
                          <input @focus="$event.target.select()" @click="$event.target.select()" type="number"
                                 class="touchspin-vertical form-control" v-model="minutes">
                          <span class="input-group-btn-vertical">
                                                <button @click="minuteAdd" style="width: 20px;"
                                                        class="btn btn-primary bootstrap-touchspin-up"
                                                        type="button">+</button>
                                                <button @click="minuteSub" style="width: 20px;"
                                                        class="btn btn-primary bootstrap-touchspin-down"
                                                        type="button">-</button>
                                            </span>
                        </div>
                    ...
                    <div class="col-4 pl-0">
                      ...
                      <div class="d-inline-block w-100">
                        <div class="input-group bootstrap-touchspin bootstrap-touchspin-injected">
                          <input @focus="$event.target.select()" @click="$event.target.select()" type="number"
                                 class="touchspin-vertical form-control" v-model="seconds">
                          <span class="input-group-btn-vertical">
                                                <button @click="secondAdd" style="width: 20px;"
                                                        class="btn btn-primary bootstrap-touchspin-up"
                                                        type="button">+</button>
                                                <button @click="secondSub" style="width: 20px;"
                                                        class="btn btn-primary bootstrap-touchspin-down"
                                                        type="button">-</button>
                                            </span>
                        </div>
                    ...
                    <div class="col-md-12" style="padding-top: 5px;">
                      <button type="button" @click="closeClick" class="btn btn-outline-primary btn-sm w-100">
                        {{ $i18n.t('trade_close') }}
                      </button>
                    </div>
                  </div>
                  ...
                  <fieldset :style="{ 'margin-bottom': '0.3rem !important'}" class="form-group position-relative">
                    <template>
                      <input ref="ci" @click="amountClick" type="text" class="form-control form-control-lg"
                             v-model="amount"
                             v-currency="{currency: null, autoDecimalMode: true, valueRange: {min: 1, max: 1000}}">
                    </template>
                    ...
                    <div v-show="clickedAmount" class="row" :style="{ 'margin-top': '0.3rem !important'}">
                      <div class="col-4 pr-0">
                        <button @click="setAmount(min)" type="button" class="btn btn-outline-primary btn-sm w-100"
                                style="padding-left: 3px;padding-right: 3px;">{{ min }}$
                        </button>
                      </div>
                      <div class="col-4" style="padding-left:7.5px;padding-right:7.5px !important;">
                        <button @click="setAmount(min*2)" type="button" class="btn btn-outline-primary btn-sm w-100"
                                style="padding-left: 3px;padding-right: 3px;">{{ min * 2 }}$
                        </button>
                      </div>
                      <div class="col-4 pl-0">
                        <button @click="setAmount(min*4)" type="button" class="btn btn-outline-primary btn-sm w-100"
                                style="padding-left: 3px;padding-right: 3px;">{{ min * 4 }}$
                        </button>
                      </div>
                    </div>
                    <div v-show="clickedAmount" class="row" :style="{ 'margin-top': '0.3rem !important'}">
                      <div class="col-4 pr-0">
                        <button @click="setAmount(min*8)" type="button" class="btn btn-outline-primary btn-sm w-100"
                                style="padding-left: 3px;padding-right: 3px;">{{ min * 8 }}$
                        </button>
                      </div>
                      <div class="col-4" style="padding-left:7.5px;padding-right:7.5px !important;">
                        <button @click="setAmount(min*16)" type="button" class="btn btn-outline-primary btn-sm w-100"
                                style="padding-left: 3px;padding-right: 3px;">{{ min * 16 }}$
                        </button>
                      </div>
                      <div class="col-4 pl-0">
                        <button @click="setAmount(min*32)" type="button" class="btn btn-outline-primary btn-sm w-100"
                                style="padding-left: 3px;padding-right: 3px;">{{ min * 32 }}$
                        </button>
                      </div>
                    </div>
                    <div v-show="clickedAmount" class="row" :style="{ 'margin-top': '0.3rem !important'}">
                      ...
                      <div class="col-4" style="padding-left:7.5px;padding-right:7.5px !important;">
                        <button @click="amountClickClose" type="button" class="btn btn-outline-success btn-sm w-100">
                          ОК
                        </button>
                      </div>
                      <div class="col-4 pl-0">
                  ...
                  <fieldset class="form-group position-relative" v-show="number_percent === null">
                    <input type="text" disabled class="form-control form-control-lg"
                           :placeholder="$i18n.t('trade_loading')" aria-describedby="basic-addon2">
                    <div class="form-control-position" :style="{ 'top' : '14px'}">
                  ...
                  <fieldset v-show="number_percent !== null" class="form-group position-relative">
                    <input type="text" class="form-control form-control-lg"
                           :value="(this.$ci.parse(this.amount) * number_percent / 100).toFixed(2)" disabled>
                    <div class="form-control-position" :style="{ 'top' : '14px'}">
                      <i class="bx bx-dollar"></i>
                    </div>
                  </fieldset>
                  ...
                  <button v-show="number_percent === 0 && percent !== ''"
                          class="disabled btn btn-success mb-1 w-100 btn-lg" type="button" disabled="disabled">
                    {{ $i18n.t('trade_closed_market') }}
                  </button>

                  <button v-show="percent !== '' && number_percent !== 0" type="button"
                          @click="placeOrder(1, '#23bd70', $i18n.t('trade_up'))"
                          class="btn btn-success glow w-100 btn-lg" v-bind:disabled="isButtonDisabled">
                  ...
                  <button v-show="number_percent === 0 && percent !== ''"
                          class="disabled btn btn-danger mb-1 w-100 btn-lg" type="button" disabled="disabled">
                    {{ $i18n.t('trade_closed_market') }}
                  </button>

                  <button v-show="percent !== '' && number_percent !== 0"
                          @click="placeOrder(0, '#FF5B5C', $i18n.t('trade_down'))" type="button"
                          class="btn btn-danger glow w-100 mt-1 btn-lg" v-bind:disabled="isButtonDisabled">
                ...
                <div class="row">
                  <div class="col-md-12" id="history">
                    <div :style="{ height: historyHeight }"
                         class="accordion collapse-icon accordion-icon-rotate ps ps--active-y" id="accordionWrapa2">
                      <!--                                            Opened orders-->
                      ...
                      <b-card no-body v-for="(open, index) in opened" :key="index" class="collapse-header">
                        <div v-b-toggle="'opened-orders' + index" data-toggle="collapse" class="card-header p-1">
                                                <span class="collapse-title">
                                                  ...
                                                  <small :class="'text-' + open.textColor"
                                                         style="float: right;padding-right: 20px;padding-top: 5px;">
                                                      <strong v-show="open.profit_status === 0" v-text="'0 $'"></strong>
                                                      <strong v-show="open.profit_status === 1"
                                                              v-text="(parseFloat(open.amount) + (open.amount * open.percent / 100)).toFixed(2) + ' $'"></strong>
                                                      <strong v-show="open.profit_status === 2"
                                                              v-text="open.amount + ' $'"></strong>
                                                 </small>

                                                  <template>
                                                      <vue-countdown-timer :start-time="'2020-01-01 00:00:00'"
                                                                           :end-time="open.timestamp" :interval="1000">
                                                          <template slot="countdown" slot-scope="scope">
                                                              <div class="progress position-relative"
                                                                   :class="'progress-bar-' + open.textColor"
                                                                   style="text-shadow: 1px 1px 2px black;">
                                                                  <div
                                                                      :style="{ width: (100 - (((scope.props.hours * 60 * 60 + scope.props.minutes * 60 + scope.props.seconds) / open.expiration) * 100)) + '%'}"
                                                                      class="progress-bar progress-bar-striped"
                                                                      role="progressbar" aria-valuenow="0"
                                                                      aria-valuemin="0" aria-valuemax="100"></div>
                                                                  <small style="margin-top: -3.4px; font-size:12px;"
                                                                         class="text-white justify-content-center d-flex position-absolute w-100">
                                                                      {{ scope.props.hours }}:{{ scope.props.minutes }}:{{ scope.props.seconds }}
                                                                  </small>

                                                                  <small style="margin-top: -3.4px; font-size:10px;"
                                                                         class="text-white justify-content-left d-flex position-absolute w-100">
                                                                      <i v-show="open.type === 1"
                                                                         class="bx bx-trending-up"
                                                                         style="font-size: 12px;padding-left: 1px;padding-top: 3px;"></i>
                                                                      <i v-show="open.type === 0"
                                                                         class="bx bx-trending-down"
                                                                         style="font-size: 12px;padding-left: 1px;padding-top: 1px;"></i>
                                                                  ...
                                                                  <small v-text="open.amount + ' $'"
                                                                         style="margin-top: -3.4px; font-size:12px; padding-right:1px;"
                                                                         class="text-white justify-content-end d-flex position-absolute w-100"></small>
                                                              </div>
                                                            </template>
                        ...
                        <b-collapse :id="'opened-orders' + index" accordion="opened-orders" role="tabpanel">
                          <div class="card-content">
                            <div class="card-body"
                                 style="padding: 15px; background-color: #22283e !important; border: 1px solid; border-top: 1px;border-bottom-left-radius: 5px; border-bottom-right-radius: 5px;">
                              {{ $i18n.t('trade_open') }}: {{ openedDate(open.created_at) }}<br>
                              {{ $i18n.t('trade_opened_price') }}: {{ parseFloat(open.open_price) }}<br>
                              {{ $i18n.t('trade_path') }}:
                              <div v-show="open.type === 1" class="badge badge-success">{{ $i18n.t('trade_up') }}</div>
                              <div v-show="open.type === 0" class="badge badge-danger">{{ $i18n.t('trade_down') }}</div>
                            </div>
                          </div>
                        </b-collapse>
                      </b-card>
                        ...
                        <ul class="list-group">
                          <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <span class="badge-circle badge-circle-warning badge-circle-sm text-white">
                                                    <i class="bx bx-info-circle font-size-base"
                                                       style="font-size: 1rem;margin-left: -1.5px;"></i>
                                                  </span>
                          </li>
                        </ul>
                      </div>
                        ...
                        <div v-b-toggle="'latest-orders' + index" data-toggle="collapse" class="card-header p-1">
                                                    <span class="collapse-title">
                                                        ...
                                                        <small :class="'text-' + open.textColor"
                                                               style="float: right;padding-right: 20px;padding-top: 5px;">
                                                            <strong
                                                                v-text="parseFloat(open.profit).toFixed(2) + ' $'"></strong>
                                                       </small>
                                                        <div class="progress position-relative"
                                                            ...
                                                            <small style="margin-top: -3.4px; font-size:10px;"
                                                                   class="text-white justify-content-left d-flex position-absolute w-100">
                                                                <i v-show="open.type === 1" class="bx bx-trending-up"
                                                                   style="font-size: 12px;padding-left: 1px;padding-top: 3px;"></i>
                                                                <i v-show="open.type === 0" class="bx bx-trending-down"
                                                                   style="font-size: 12px;padding-left: 1px;padding-top: 1px;"></i>
                                                                <span class="text-white" style="padding-top: 1px;">{{
                                                                    open.percent
                                                                  }}%</span>
                                                            </small>
                      ...
                      <div class="text-center mb-2">
                        <span v-show="!historyLoaded" role="status" aria-hidden="true"
                              class="spinner-border spinner-grow-sm" style="width:3rem;height:3rem;"></span>
                      </div>
                      <router-link to="/history">
                        <template>
                          <button type="button" class="btn btn-outline-primary w-100">
                            {{ $i18n.t('trade_full_orders_history') }}
                          </button>
                        </template>
                      </router-link>
                      <!--                                        End latest orders-->
...
export default {
      ...
      this.opened.forEach((open) => {
        if (this.symbols.length > 0) {
          try {
            ...
            if ('lp' in this.fastData[symbol.broker + ':' + symbol.symbol.replace('/', '')]) {
    ...
    placeOrder: function (direction, color, text) {
      ...
      axios.post('/binary/buy', {
        ...
        id: this.contestId != null ? this.contestId : 0,
          ...
          .catch(error => {
            toastr.error(error.response.data.message, this.$i18n.t('trade_error'), {
              positionClass: 'toast-bottom-left',
              containerId: 'toast-bottom-left'
            })
          });
  ...
  data() {
    return {
      ...
      minutes: localStorage.getItem('minutes') ? localStorage.getItem('minutes') : '01',
